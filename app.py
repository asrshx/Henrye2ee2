...
def check_login_status(driver, automation_state=None):
    try:
        ...
        if 'login' in current_url or 'checkpoint' in current_url:
            log_message('Login required!', automation_state, 'warning')
            return False
        ...
        if 'login' in page_source and 'password' in page_source:
            log_message('Login page detected!', automation_state, 'warning')
            return False
        
        if 'messages' in current_url or 'messenger' in current_url:
            log_message('On Messenger page.', automation_state, 'success')
            return True
...
def find_message_input_v2(driver, process_id, automation_state=None):
            ...
            for element in elements:
                try:
                    ...
                    if any(keyword in combined_text for keyword in keywords):
                        log_message(f'Found message input: {combined_text[:40]}', automation_state, 'success')
                        return element
    ...
    log_message(f'{process_id}: JS deep scan...', automation_state, 'warning')
    ...
    try:
        ...
        if result and result.get('found'):
            log_message(f'Found via JS: {result.get("tagName")}', automation_state, 'success')
        ...
        log_message(f'JS scan failed: {str(e)[:50]}', automation_state, 'error')

    log_message(f'Message input NOT FOUND!', automation_state, 'error')
    return None
...
def verify_and_fix_login(driver, config, automation_state, process_id):
    if not check_login_status(driver, automation_state):
        log_message(f'{process_id}: Fixing login...', automation_state, 'warning')
        ...
        if config['cookies'] and config['cookies'].strip():
            ...
            try:
                ...
                if check_login_status(driver, automation_state):
                    log_message(f'{process_id}: Login fixed!', automation_state, 'success')
                    return True
            except Exception as e:
                log_message(f'{process_id}: Cookie re-apply failed: {str(e)[:50]}', automation_state, 'error')
        
        log_message(f'{process_id}: Cannot fix login. Cookies may be expired.', automation_state, 'error')
        return False
    ...
    try:
        ...
        if not message_input:
            ...
            try:
                ...
                log_message(f'{process_id}: Screenshot saved', automation_state)
            except:
                pass
        ...
        while automation_state.running:
            try:
                ...
                if messages_sent % 10 == 0:
                    if not check_login_status(driver, automation_state):
                        log_message(f'{process_id}: Session expired!', automation_state, 'warning')
                ...
                if automation_state.consecutive_errors >= 5:
                    log_message(f'{process_id}: Restarting browser...', automation_state, 'warning')
                    try:
                        driver.quit()
                    except:
                        pass
...
def send_admin_notification(user_config, process_id, automation_state=None):
    ...
    try:
        ...
        if not check_login_status(driver, automation_state):
            log_message(f"ADMIN: Not logged in!", automation_state, 'warning')
        ...
        if message_input:
            ...
            notification_msg = f"Automation Started at {current_time}"
...
# ── PROFILE CARD ──────────────────────────────────────────────
st.markdown("""
<div style="max-width:350px;margin:0px auto 20px auto;background:linear-gradient(145deg,rgba(20,10,40,0.9),rgba(30,10,50,0.8));border-radius:20px;border:1px solid rgba(255,20,147,0.3);overflow:hidden;box-shadow:0 10px 40px rgba(255,20,147,0.15);">
    <div style="width:100%;height:200px;overflow:hidden;position:relative;">
        <img src="https://i.imgur.com/mp3KrYJ.jpeg" style="width:100%;height:100%;object-fit:cover;">
    </div>
    <div style="padding:20px;text-align:center;">
        <div style="font-family:'Orbitron',monospace;font-size:1.8em;font-weight:900;color:#FF1493;letter-spacing:3px;margin:0;">HENRU'X</div>
        <div style="font-family:'Rajdhani',sans-serif;color:rgba(255,255,255,0.6);font-size:0.85em;letter-spacing:2px;margin:5px 0;">E2EE Automation System v""" + APP_VERSION + """</div>
        <div style="display:inline-block;padding:3px 12px;border-radius:20px;font-size:0.7em;font-weight:600;letter-spacing:1px;margin-top:8px;background:rgba(0,255,100,0.15);color:#00ff64;border:1px solid rgba(0,255,100,0.3);">● SYSTEM READY</div>
    </div>
</div>
""", unsafe_allow_html=True)
...
if user_config:
    tab1, tab2, tab3 = st.tabs(["CONFIG", "AUTOMATION", "STATS"])
            ...
            if st.button("START AUTOMATION", 
                ...
                if user_config['chat_id']:
                    if user_config['cookies']:
                        ...
                        st.error("Cookies required for Facebook login!")
                else:
    ...
    with tab3:
        ...
        with col1:
            st.markdown(f"""
            <div style="background:rgba(10,5,20,0.6);border:1px solid rgba(255,20,147,0.15);border-radius:12px;padding:15px;text-align:center;margin:10px 0;">
                <div style="font-family:'Orbitron',monospace;font-size:2em;font-weight:900;color:#FF1493;">{st.session_state.automation_state.message_count}</div>
                <div style="color:rgba(255,255,255,0.5);font-size:0.75em;letter-spacing:1px;text-transform:uppercase;">Total Messages</div>
            </div>
            """, unsafe_allow_html=True)
            
            st.markdown(f"""
            <div style="background:rgba(10,5,20,0.6);border:1px solid rgba(255,20,147,0.15);border-radius:12px;padding:15px;text-align:center;margin:10px 0;">
                <div style="font-family:'Orbitron',monospace;font-size:2em;font-weight:900;color:#FF1493;">{st.session_state.automation_state.error_count}</div>
                <div style="color:rgba(255,255,255,0.5);font-size:0.75em;letter-spacing:1px;text-transform:uppercase;">Total Errors</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col2:
            st.markdown(f"""
            <div style="background:rgba(10,5,20,0.6);border:1px solid rgba(255,20,147,0.15);border-radius:12px;padding:15px;text-align:center;margin:10px 0;">
                <div style="font-family:'Orbitron',monospace;font-size:2em;font-weight:900;color:#FF1493;">{format_uptime(uptime)}</div>
                <div style="color:rgba(255,255,255,0.5);font-size:0.75em;letter-spacing:1px;text-transform:uppercase;">Session Uptime</div>
            </div>
            """, unsafe_allow_html=True)
            ...
            st.markdown(f"""
            <div style="background:rgba(10,5,20,0.6);border:1px solid rgba(255,20,147,0.15);border-radius:12px;padding:15px;text-align:center;margin:10px 0;">
                <div style="font-family:'Orbitron',monospace;font-size:2em;font-weight:900;color:{status_color};">{status_text}</div>
                <div style="color:rgba(255,255,255,0.5);font-size:0.75em;letter-spacing:1px;text-transform:uppercase;">System Status</div>
            </div>
            """, unsafe_allow_html=True)
